...
function Keypad() {
	...
	this._defaults = { // Global defaults for all the keypad instances
		...
		showInput: true, // True to show the Input
		showPrompt: true, // True to show the prompt message
...
$.extend(Keypad.prototype, {
	...
	_setInput: function(target, inst) {
		var showInput = this._get(inst, 'showInput');
		var showInputCss = "";
		if (!showInput){
			showInputCss = 'style="display: none;"';
		}
		inst._input = $(!inst._inline ? target : this._get(inst, 'target') ||
			'<input type="text" class="' + this._inlineEntryClass + '" '+ showInputCss + ' disabled="disabled"/>');
		if (inst._inline) {
			target = $(target);
	...
	_generateHTML: function(inst) {
		...
		var showPrompt = this._get(inst, 'showPrompt');
		var separator = this._get(inst, 'separator');
		var html = "";
		if (showPrompt){
			html = (!prompt ? '' : '<div class="keypad-prompt' +
